...
// GetDataList 获取玩家
func (b *PtBaseData) GetDataList(userid int, startTime, endTime string, gold,
	page, limit int, isPost bool) (tempData []PtBaseData, pag Pagination, err error) {
	o := newOrmBy(PTDIV)
	tempData = []PtBaseData{}
	query := o.QueryTable(prefixBy(PTDIV) + b.TableName())
	if userid != 0 {
...
// UpdateBase 更新玩家数据
func (b *PtBaseData) UpdateBase() (err error) {
	o := newOrmBy(PTDIV)
	query := o.QueryTable(prefixBy(PTDIV) + b.TableName())
...
// Remove 删除玩家数据
func (b *PtBaseData) Remove() (err error) {
	o := newOrmBy(PTDIV)
...
// GetData 获取玩家
func (b *PtBaseFu) GetData() (err error) {
	o := newOrmBy(PTDIV)
	query := o.QueryTable(prefixBy(PTDIV) + b.TableName())
...
// UpdateBase 更新玩家数据
func (b *PtBaseFu) UpdateBase() (err error) {
	o := newOrmBy(PTDIV)
	query := o.QueryTable(prefixBy(PTDIV) + b.TableName())
	query = query.Filter("id", b.ID)
	num, err := query.Update(orm.Params{
...
// GetDataList 获取玩家
func (b *PtBaseFu) GetDataList(userid int, startTime, endTime string, ticket,
	page, limit int, isPost bool) (tempData []PtBaseFu, pag Pagination, err error) {
	o := newOrmBy(PTDIV)
	tempData = []PtBaseFu{}
	query := o.QueryTable(prefixBy(PTDIV) + b.TableName())
	if userid != 0 {
...
//update em_base_rank_num set num = num + 1 where num > 0 and score in (select score from pt_base_rank where userid = 1)
//update em_base_rank_num as a inner join (select score from pt_base_rank where userid = 1) as c on a.score = c.score set a.num = a.num - 1 where a.num > 0;
// RankRemove 删除玩家rank数据
func (b *PtBaseData) RankRemove() (err error) {
	o := newOrmBy(PTDIV)
	tableName := "pt_base_rank"
	tableName2 := "pt_base_rank_num"
